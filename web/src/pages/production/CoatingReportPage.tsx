...
import { Calendar, ChevronDown, Download, Paintbrush } from 'lucide-react'
...
function exportCSV(rows: any[], from: string, to: string, contractorName: string) {
  const header = ['PO Number', 'Pipe Config', 'Dia (mm)', 'Pipes Coated', 'Rate (₹/pipe)', 'Amount (₹)']
  const data = rows.map(r => [r.poNumber, r.pipeConfig, r.diameterMm, r.coatPipesCompleted, fmt(r.coatRatePerPipe), fmt(r.coatCost)])
  const total = rows.reduce((s, r) => s + Number(r.coatCost), 0)
  data.push(['', '', '', '', 'TOTAL', fmt(total)])
  const csv = [header, ...data].map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(',')).join('\n')
  const blob = new Blob([`Coating Report\nContractor: ${contractorName}\nPeriod: ${from||'All'} to ${to||'All'}\n\n${csv}`], { type: 'text/csv' })
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob)
  a.download = `coating-report-${from||'all'}-to-${to||'all'}.csv`; a.click()
  URL.revokeObjectURL(a.href)
}

export default function CoatingReportPage() {
  ...
  const contractor = (assignments as any[]).find((a: any) => a.processType === 'COATING')
  const contractorName = contractor?.supplier?.name ?? 'Coating Contractor'

  const rows  = (data ?? []).filter((r: any) => Number(r.coatPipesCompleted) > 0)
  const total = rows.reduce((s: number, r: any) => s + Number(r.coatCost), 0)
  const totalPipes = rows.reduce((s: number, r: any) => s + Number(r.coatPipesCompleted), 0)
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-700 via-blue-600 to-cyan-600 px-8 pt-7 pb-6 shadow-[0_8px_40px_rgba(29,78,216,0.30)]">
        ...
        <div className="relative mt-5 flex items-stretch gap-px bg-white/10 rounded-xl overflow-hidden border border-white/10">
          {[
            ...
            { label: 'Pipes Coated',     value: totalPipes.toLocaleString() },
      ...
      <div className="p-6">
        <div className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide">Coating Entries</h2>
            <span className="text-xs text-gray-400 font-medium">Rate: ₹/pipe by diameter</span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              ...
              <p className="font-semibold text-gray-500">No coating entries found</p>
