...
import {
  ...
  Package, Tag, CalendarDays, Info, Save,
} from 'lucide-react'
      ...
      <div className="flex-1 min-w-0">
        ...
        {value.creditLimit > 0 && (
          <p className="text-xs text-violet-600 font-medium mt-0.5">
            Credit: ₹{parseFloat(value.creditLimit).toLocaleString('en-IN')} · Due: ₹{parseFloat(value.outstandingDue ?? 0).toLocaleString('en-IN')}
          </p>
        )}
      </div>
      <button onClick={onClear} className="text-xs text-gray-400 hover:text-red-500 shrink-0">Change</button>
    </div>
  )
  ...
  function addProduct(p: any) {
    ...
    setItems(prev => {
      ...
      const newItem: LineItem = {
        ...
        lineTotal: calcLine({ productId: p.id, productName: p.name, sku: p.sku, unitPrice, quantity: 1, discountPercent: 0, taxRate }),
      }
      return [...prev, newItem]
    })
  }
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-slate-50 via-blue-50/20 to-violet-50/20"
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4 sticky top-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900 leading-none">
              Edit — <span className="font-mono text-violet-700">{so?.soNumber}</span>
            </h1>
        ...
        <div className="ml-4 flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-1.5">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-5 gap-5">
        ...
        <div className="col-span-3 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-violet-50 to-blue-50 border-b border-gray-100 px-5 py-3 flex items-center justify-between">
              ...
              <span className="text-xs bg-violet-100 text-violet-700 font-semibold px-2.5 py-1 rounded-full">{items.length} item{items.length !== 1 ? 's' : ''}</span>
            </div>
            <div className="p-5 space-y-4">
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-gray-400 border-2 border-dashed border-gray-100 rounded-xl">
                  ...
                  <p className="text-sm">No items — search and add products above</p>
                </div>
              ) : (
        ...
        <div className="col-span-2 space-y-4">
          ...
          {/* Save button */}
          <button onClick={handleSave} disabled={saving || !customer || items.length === 0 || creditWarning}
            className="w-full py-3.5 bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-700 hover:to-blue-700 disabled:opacity-50 text-white font-semibold rounded-2xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-violet-200 text-sm">
            {saving
